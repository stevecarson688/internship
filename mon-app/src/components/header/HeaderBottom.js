...
const HeaderBottom = () => {
    ...
    return (
        <div className="w-full px-4 h[36px] bg-amazon_bl text-white flex items-center">
            ...
            {
                sidebar && (
                    <div className="w-full h-screen text-black fixed top-0 left-0 bg-amazon_white bg-opacity-50">
                        <div className="w-full h-full relative">
                            <motion.div ref={ref} initial={{ x: -500, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .5 }} className="w-[80%] md:w-[350px] h-full bg-white border  border-amazon_bl">
                                <div className="w-full bg-amazon_bl text-white py-2 px-6 flex items-center gap-4">
                                    <AccountCircleIcon />

                                    {
                                        useInfo ? (
                                            <h3 className="font-titleFont font-bold text-lg tracking-wide">{useInfo.userName}</h3>
                                        ) : (
                                ...
                                <div>
                                    ...
                                    <span onClick={() => setSidebar(false)}
                                        className="cursor-pointer absolute top-0 left-[360px] w-10 h-10 text-black flex items-center justify-center border bg-gray-200 hover:bg-blue-500 hover:text-white duration-300">
                                        <CloseIcon />
                                    </span>
                                </div>
